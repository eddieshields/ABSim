...
{
  debug() << "Building PhaseSpace" << endmsg;
...
{
  debug() << "Building Resonances" << endmsg;
  ...
  if ( type == "RBW" ) {
    ...
    warning() << "Unrecognised type " << type << endmsg;
  }
  return;
}
...
{
  debug() << m_config.get(name) << endmsg;
  std::stringstream res( m_config.get(name) );
